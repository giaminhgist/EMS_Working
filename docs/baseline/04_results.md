...
(`metrics.json` is committed; per-subject predictions and the 48-subject
`official_test_preds.csv` are kept out of the repository — they are
regenerated by the scripts and are intended for benchmark submission only).
